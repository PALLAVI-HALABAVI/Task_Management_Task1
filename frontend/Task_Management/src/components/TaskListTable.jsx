...
const TaskListTable = ({tableData}) => {
    const getStatusBadgeColor = (status) => {
        switch (status){
            case 'Completed': return 'bg-pink-100 text-pink-500 border border-gray-200';
            case 'Pending' : return 'bg-lime-100 text-lime-500 border border-gray-200';
            case 'In Progress' : return 'bg-cyan-100 text-cyan-500 border border-gray-200';
            ...
            <tbody>
                {tableData.map((task) => (
                    <tr key={task._id} className='border-t border-gray-200'>
                        <td className='my-3 mx-4 text-gray-700 text-[13px] lne-clamp-1 overflow-hidden'>
                            {task.title}
                        </td>
